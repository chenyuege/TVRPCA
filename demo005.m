...
%   4. LRTV
%   5. EnhancedRPCA 
% three quality assessment (QA) in      dices -- PSNR, SSIM, ERGAS
...
% [1] paper
%
% by Jiangjun peng, 2019.
%% ==================================================================
clear all;
clc;
addpath(genpath('src/compared methods/'))
addpath(genpath('src/EnhancedRPCA/'))
addpath(genpath('data/CAVE/'))
...
MMERGAS ={};
MMTIME  = {};
for dataid = 1:length(Test_file)
    disp(['reading the ',Test_file{dataid}]);
    fprintf('\n');
    Omsi      =    load_HSI( Dir, Test_file{dataid});% load the normalize data,[0,1]
    Nmsi    = Omsi;
    [M,N,p] = size(Omsi);
    %% ===========================noise simulated=======================
    GaussianSwitch     = 1;
    ...
    DeadlineSwitch     = 0; 
    StripSwitch        = 0;
    noiselevel   = 0.05*ones(p,1);%0.075*ones(p,1);0.05+0.1*rand(p,1);
